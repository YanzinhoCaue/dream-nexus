...
const DreamModal = ({ isOpen, onClose, nodeData, onSave }) => {
  ...
  const handleSave = () => {
    // Só salva se não estiver carregando a imagem
      ...
      <div className="bg-[#05050a] w-full max-w-3xl h-[90vh] rounded-none border border-cyan-500/50 shadow-[0_0_100px_rgba(0,229,255,0.15)] flex flex-col overflow-hidden relative">
        ...
        <div className="p-6 border-b border-cyan-900/50 flex justify-between items-center bg-cyan-950/10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-cyan-400 font-oxanium text-sm tracking-[0.2em] uppercase">
              System Configuration // {nodeData.id}
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
          ...
          <div className="space-y-3">
            <label className="text-xs text-cyan-600 uppercase tracking-widest font-bold">
              Visual Database
            </label>
            ...
            <div
              ...
              className={`relative w-full h-48 border-2 border-dashed ${
                isUploading
                  ? "border-cyan-500 bg-cyan-900/20 cursor-wait"
                  : "border-gray-700 hover:border-cyan-500 hover:bg-cyan-900/10 cursor-pointer"
              } rounded-lg transition-all flex flex-col items-center justify-center group overflow-hidden`}>
              {isUploading ? (
                <div className="flex flex-col items-center gap-2">
                  <Loader2 className="animate-spin text-cyan-400" size={32} />
                  <span className="text-xs text-cyan-400 font-bold animate-pulse">
                    UPLOADING ASSET...
                  </span>
                </div>
              ) : imagePreview ? (
                ...
                <div className="flex flex-col items-center text-gray-500 group-hover:text-cyan-400 transition-colors">
                  ...
                  <span className="text-sm font-oxanium uppercase tracking-widest">
                    Upload from System
                  </span>
                </div>
              )}
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end border-b border-gray-800 pb-2">
              <h4 className="text-cyan-400 font-oxanium uppercase text-sm tracking-widest">
                Execution Steps
              </h4>
            ...
            <div className="space-y-2">
              {steps.map((step, index) => (
                <div
                  key={step.id}
                  className="group flex items-center gap-4 bg-gray-900/50 p-4 border-l-2 border-transparent hover:border-cyan-500 hover:bg-cyan-950/10 transition-all">
                  {/* Controles */}
                  ...
                  <button
                    ...
                    {step.done ? (
                      <CheckCircle
                        className="text-cyan-400 drop-shadow-[0_0_5px_#00E5FF]"
                        size={20}
                      />
                    ) : (
                      <Circle
                  ...
                  <input
                    ...
                    className={`flex-1 bg-transparent focus:outline-none font-mono text-sm ${
                      step.done
                        ? "text-gray-600 line-through decoration-cyan-900"
                        : "text-gray-300"
                    }`}
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-red-900 hover:text-red-500 transition-all">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
        ...
        <div className="p-6 border-t border-cyan-900/30 flex justify-end gap-4 bg-black/40">
          <button
            onClick={onClose}
            className="px-6 py-3 text-gray-500 hover:text-white font-oxanium text-sm uppercase tracking-wider transition-colors">
            Abort
          </button>
          <button
            onClick={handleSave}
